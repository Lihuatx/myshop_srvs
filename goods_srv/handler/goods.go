...
// GoodsList 方法用于处理商品列表查询请求。
// 它根据传入的过滤条件，使用 Elasticsearch 和 MySQL 数据库来检索和返回符合条件的商品信息。
//
// 参数：
//
//	ctx: 请求的上下文对象，用于控制请求的生命周期。
//	req: 指向 proto.GoodsFilterRequest 结构体的指针，包含查询请求的各种过滤条件。
//
// 返回值：
//
//	*proto.GoodsListResponse: 指向 proto.GoodsListResponse 结构体的指针，包含查询结果。
//	error: 如果查询过程中出现错误，则返回错误信息；否则返回 nil。
