...
import (
	...
	"gorm.io/gorm"
	"myshop_srvs/inventory_srv/model"

	"myshop_srvs/inventory_srv/global"
	"myshop_srvs/inventory_srv/proto"
)
...
// Sell 方法用于处理库存扣减操作。
// 该方法接收一个上下文对象和一个包含销售信息的请求，
// 并在库存充足的情况下执行库存扣减操作，同时记录销售详情。
//
// 参数：
//
//	ctx: 上下文对象，用于控制请求的生命周期和取消信号。
//	req: 指向 proto.SellInfo 结构体的指针，包含销售信息，如订单号和商品详情。
//
// 返回值：
//
//	*emptypb.Empty: 一个空的响应消息，表示操作成功完成。
//	error: 如果操作过程中出现错误，则返回错误信息；否则返回 nil。
//
// 说明：
//  1. 方法首先创建一个 Redis 客户端和连接池，用于实现分布式锁。
//  2. 启动一个数据库事务以确保操作的原子性。
//  3. 对于请求中的每个商品，方法首先尝试获取一个基于商品 ID 的分布式锁。
//  4. 如果成功获取锁，则查询库存信息，并检查库存是否充足。
//  5. 如果库存充足，则扣减库存并更新库存记录。
//  6. 释放锁并记录销售详情。
//  7. 如果在任何步骤中发生错误，则回滚事务并返回错误。
//  8. 如果所有操作都成功完成，则提交事务并返回成功响应。
func (*InventoryServer) Sell(ctx context.Context, req *proto.SellInfo) (*emptypb.Empty, error) {

	client := goredislib.NewClient(&goredislib.Options{
		Addr: "192.168.10.108:6379",
	})
